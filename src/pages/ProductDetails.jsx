  ...
  if (!product) {
    return (
       <div className="min-h-screen pt-32 text-center text-white px-6">
          ...
          <button onClick={() => navigate('/shop')} className="text-white border-b border-white">Back to Shop</button>
       </div>
    );
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20">
          ...
          <div className="space-y-4">
            <motion.div 
              ...
              className="aspect-square bg-zinc-900 rounded-2xl overflow-hidden border border-white/5"
            >
              <img 
            ...
            <div className="grid grid-cols-4 gap-4">
              {product.images.map((img, idx) => (
                <div key={idx} className="aspect-square bg-zinc-900 rounded-lg overflow-hidden border border-white/5 cursor-pointer hover:border-white/40 transition-colors">
          ...
          <div>
            ...
            <p className="text-2xl text-white font-medium mb-6">₹{currentPrice}</p>
            ...
            <div className="space-y-8 mb-10">
              ...
              {product.variants.some(v => v.name !== 'Standard') && (
                <div>
                  <label className="block text-sm font-medium text-white mb-3">Option</label>
                  <div className="flex flex-wrap gap-3">
                    {Array.from(new Set(product.variants.map(v => v.name))).map(optionName => (
                      <button
                        key={optionName}
                        onClick={() => {
                          const matchingVariant = product.variants.find(v => v.name === optionName);
                      ...
                      .map(variant => (
                        <button
                          ...
                          className={cn(
                            ...
                            selectedVariantId === variant.id ? "border-white scale-110" : "border-transparent hover:scale-105"
                          )}
                          style={{ backgroundColor: variant.colorHex || '#333' }}
                        >
                          {selectedVariantId === variant.id && (
                            <Check size={16} className={(variant.colorHex === '#f0f0f0' || !variant.colorHex) ? 'text-black' : 'text-white'} />
                          )}
                        </button>
                    ))}
                  </div>
               ...
               {isOutOfStock ? (
                 ...
                 <div className="flex gap-4">
                   ...
                   <button 
                     ...
                     className="flex-1 py-3 bg-white text-black font-bold rounded-full hover:bg-white/90 transition-all flex items-center justify-center gap-2"
                   >
